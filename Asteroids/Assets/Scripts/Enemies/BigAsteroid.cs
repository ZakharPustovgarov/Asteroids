...
public class BigAsteroid : SmallAsteroid
{
    ...
    public override void TakeDamage(string damageType)
    {
        if(damageType != "Laser")// Если урон получен не от лазера, то
        {
            ...
            {
                SmallAsteroid asteroid = GameObject.Instantiate(smallAsteroidPrefab, this.transform.position, this.transform.rotation).GetComponent<SmallAsteroid>();// Создаётся объект осколка
